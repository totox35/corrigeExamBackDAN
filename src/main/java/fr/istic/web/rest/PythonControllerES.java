...
public class PythonControllerES {
    ...
    @GET
    @Path("/get-related-chunks")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response getRelatedChunks(@QueryParam("query") String query,
        ...
        try {
            log.info("Starting the process to get related chunks...");

            // Validate the query parameter
            if (query == null || query.isEmpty()) {
                log.error("Query text missing in the request.");
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(Map.of("error", "Query text is missing."))
                        .build();
            }
            ...
            // Path to Python script
            String scriptPath = "src/main/resources/rag/get_related_chunks.py";
            ...
            // Run Python script with query, courseName and topN as arguments
            ProcessBuilder pb = new ProcessBuilder(
                    "python3",
                    scriptFile.getAbsolutePath(),
                    query,
